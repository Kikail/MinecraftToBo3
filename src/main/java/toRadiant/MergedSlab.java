...
public class MergedSlab {
    ...
    public MergedSlab(Position min, Position max, String texture, int id, boolean isBottom) {
        this.min = min;
        this.max = max;
    ...
    public static List<MergedSlab> greedyMerge(Map<Position, Slab> blockMap) {
        Set<Position> visited = new HashSet<>();
        List<MergedSlab> MergedBlocks = new ArrayList<>();
        ...
        for (Position pos : positions) {
            ...
            Slab start = blockMap.get(pos);
            int identifiant = start.identifiant;
            String texture = start.texture;
            boolean isBottom = start.isBottom;

            int maxX = pos.x;
            while (true) {
                Position next = new Position(maxX + 1, pos.y, pos.z);
                Slab b = blockMap.get(next);
                if (b != null && !visited.contains(next) && b.texture.equals(texture) && b.isBottom == isBottom) {
                    maxX++;
                } else {
                    break;
                }
            ...
            MergedBlocks.add(new MergedSlab(min, max, texture, identifiant, start.isBottom));
        }
